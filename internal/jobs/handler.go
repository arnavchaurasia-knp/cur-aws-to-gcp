...
const (
	...
	projectionAuditDir = "projection-audit"
	projectionDuckDB   = "projection.duckdb"
)
...
// isValidJobID returns true only for canonical UUID-format job IDs.
// All job IDs are generated by uuid.New() in Create; rejecting non-UUIDs
// prevents path traversal before any filesystem access.
...
func resolveReportPath(jobDir, runIDParam string) (string, string) {
	...
	if p := filepath.Join(jobDir, projectionAuditDir, "report.html"); fileExists(p) {
		return p, ""
	}
...
func (h *Handler) Runs(w http.ResponseWriter, r *http.Request) {
	...
	dbPath := filepath.Join(h.jobsDir, id, projectionAuditDir, projectionDuckDB)
